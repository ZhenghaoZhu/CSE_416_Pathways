...
class coursePlan extends Component {
    ...
    addCourse = async function (key, studentMap) {
        ...
        if (courseInfo === undefined || courseInfo["data"] === undefined || courseInfo["data"].length === 0 || courseInfo === null) {
    ...
    nextSteps(courseInfo, studentMap, key) {
        ...
        if (timeSlot[0] !== undefined && timeSlot[1] !== undefined) {
            ...
            this.coursePl[this.sem + " " + this.year][this.coursePl[this.sem + " " + this.year].length] = [
                timeSlot[0],
                key,
                timeSlot[1],
                courseInfo["data"][0]["credits"],
            ];
    ...
    meetPrereq(info, studentMap) {
        ...
        if (info[0].length === 0) {
            ...
            for (var j = 0; j < req.length; j++) {
                for (var key of Object.keys(this.coursePl)) {
                    //go through all courses taken by the student already
