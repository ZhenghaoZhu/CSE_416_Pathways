...
import { VictoryBar, VictoryChart, VictoryGroup, VictoryContainer, VictoryTooltip } from "victory";
		...
		return (
			<>
				<VictoryChart height={500} width={800} containerComponent={<VictoryContainer responsive={false} />}>
					<VictoryGroup offset={this.props.scaleFactor} labelComponent={<VictoryTooltip />} colorScale={"qualitative"}>
						{this.props.victoryObjectArray.map((elem) =>
							elem.map((subElem, index) => (
								<VictoryBar barWidth={(1 / this.props.scaleFactor) * 20} key={index} data={subElem} />
							))
						)}
					</VictoryGroup>
				</VictoryChart>
		...
		return (
			<>
				<form onSubmit={this.handleCourseSubmit}>
					<label>
						Select Course
						<select value={course} onChange={this.handleCourseChange}>
				...
				<form>
					<label>
						Select Semester
						<select value={semester} onChange={this.handleSemesterChange}>
				...
				<br />
				<button onClick={this.handleYearAndSemesterSubmit}>Add Term to Query</button>
...
class EnrollmentTrends extends Component {
	...
	render() {
		...
		const victoryObjectArray = this.createVictoryObjectArray(enrollmentTrendsForChart);
		...
		return (
			<>
				...
				<EnrollmentChart victoryObjectArray={victoryObjectArray} scaleFactor={victoryObjectArray.length} />
			</>
		);
	}
}
